...
def test_optdep():
    ...
    if pl:
        ...
        assert test.series_equal(ref)

        # using replace_exact
        test = countrycode(pl.Series(inp_cctld), origin='cctld', destination='country.name')
    ...
    if pd:
        ...
        # using replace_exact
        test = countrycode(pd.Series(inp_cctld), origin='cctld', destination='country.name')
        ref = pd.Series(out)
        assert isinstance(test, pd.Series)
        assert pd.Series.compare(test, ref).empty
